...
def _compile_direct_matched_node_relationship_write_sql(
    *,
    ...
    _alias, from_sql, where_parts, _node_type, left_endpoint, right_endpoint = (
...
def _compile_resolved_match_relationship_write_sql(
    *,
    ...
    from .compile import _require_single_relationship_type, _resolve_type_aware_property_column
    ...
    *,
    ...
    if left.alias in matched_aliases and right.alias in matched_aliases:
        if all_matched_sql is None:
            raise ValueError(
                "Expected concrete all-matched SQL for a fully matched "
                "relationship write."
            )
    ...
    if guard_relationship_merge:
        ...
        exists_joins = [
            (
                f"JOIN {existing_node_table_sql} AS {existing_node_alias} "
                f"ON {existing_node_alias}.id = "
                f"{existing_edge_alias}.{new_column}"
            )
        ]
        for field, value in relationship.properties:
            exists_where.append(

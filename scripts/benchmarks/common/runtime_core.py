...
from scripts.benchmarks.common.runtime_postgresql_backend import (
    _analyze_postgresql,
...
@dataclass(slots=True)
class _RSSStageAccumulator:
    client: _RSSMetricAccumulator
    server: _RSSMetricAccumulator
    combined: _RSSMetricAccumulator

    @classmethod
    def create(cls) -> _RSSStageAccumulator:
        return cls(
            client=_RSSMetricAccumulator(),
            server=_RSSMetricAccumulator(),
            combined=_RSSMetricAccumulator(),
    ...
    def add(self, snapshot: dict[str, float | None]) -> None:
        self.client.add(snapshot.get("client_mib"))
        self.server.add(snapshot.get("server_mib"))
        self.combined.add(snapshot.get("combined_mib"))

    def summary(self) -> dict[str, float | None]:
        return {
            "client_mean_mib": self.client.mean(),
            "client_peak_mib": self.client.peak,
            "server_mean_mib": self.server.mean(),
            "server_peak_mib": self.server.peak,
            "combined_mean_mib": self.combined.mean(),
            "combined_peak_mib": self.combined.peak,
        }
...
def _measure_query(
    ...
    *,
    ...
    rss_samples_by_stage = {
        "before_compile": _RSSStageAccumulator.create(),
        "after_compile": _RSSStageAccumulator.create(),
        "after_execute": _RSSStageAccumulator.create(),
        "after_reset": _RSSStageAccumulator.create(),
    }
    ...
    try:
        for iteration_index in range(1, iterations + 1):
            ...
            for stage_name, stage_snapshot in metrics["rss_stages_mib"].items():
                rss_samples_by_stage[stage_name].add(stage_snapshot)
    finally:
        if gc_was_enabled:
            gc.enable()
    ...
    return {
        ...
        "rss_stages_mib": {
            stage_name: stage_samples.summary()

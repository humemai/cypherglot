...
_CONFORMANCE_QUERIES: dict[str, str] = {
    ...
    "not_eq": "MATCH (u:User) WHERE NOT u.age = 30 RETURN u.name AS name ORDER BY name",
    "not_in": "MATCH (u:User) WHERE NOT u.name IN ['Alice', 'Bob'] RETURN u.name AS name ORDER BY name",
    "not_starts_with": "MATCH (u:User) WHERE NOT u.name STARTS WITH 'A' RETURN u.name AS name ORDER BY name",
    "and_not": "MATCH (u:User) WHERE u.active = true AND NOT u.age = 30 RETURN u.name AS name ORDER BY name",
